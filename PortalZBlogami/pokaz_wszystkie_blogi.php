...
<?php
	...
	if($polaczenie->connect_errno)
	{
		...
		$sql = "SELECT w.date, w.tytul, w.opis, w.tresc, w.image_path, w.file_tresc_path, u.login FROM wpisy AS w INNER JOIN uzytkownicy AS u ON w.id_user = u.id ORDER BY w.id DESC";
		...
		if($rezultat = $polaczenie->query($sql)){
			...
			if($ile_wynikow>0){
				...
				while($wiersz = $rezultat->fetch_assoc()){
					...
					$tresc_path = $wiersz['file_tresc_path'];
						...
						<img id='img' src=' ".$img_path."' class='card-img-top' alt='...'>
							<div class='card-body'>
								<div class='top pb-4 d-flex justify-content-between'>
									...
									<a href=".$tresc_path."><h5 class='card-title'>".$tytul."</h5></a>
